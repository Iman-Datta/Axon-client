...
import { GitBranch, GitCommitHorizontal, GitPullRequest, GitMerge } from "lucide-react";

const baseTickets = [
  { id: 101, title: "Authentication UI", status: "todo", priority: "medium", assignee: "Ankita Rao", points: 3 },
  { id: 104, title: "Implement API authentication", status: "todo", priority: "high", assignee: "Rahul Menon", points: 5 },
  { id: 108, title: "Dashboard improvements", status: "review", priority: "low", assignee: "Iman Sheikh", points: 2 },
  { id: 96, title: "Database setup", status: "done", priority: "medium", assignee: "Devika N", points: 8 },
];

const steps = [
  { status: "todo", event: "Ticket created", detail: "#104 · backlog", icon: GitCommitHorizontal, tone: "text-muted-foreground" },
  { status: "dev", event: "Branch created", detail: "feature/ticket-104", icon: GitBranch, tone: "text-dev" },
  { status: "dev", event: "Commit pushed", detail: '"Implement JWT authentication"', icon: GitCommitHorizontal, tone: "text-dev" },
  { status: "review", event: "Pull request opened", detail: "#42 · axon/api", icon: GitPullRequest, tone: "text-review" },
  { status: "done", event: "Pull request merged", detail: "#42 merged into main", icon: GitMerge, tone: "text-done" },
];
...
export function Hero() {
  ...
  useEffect(() => {
    const t = setInterval(() => setStep((s) => (s + 1) % (steps.length + 1)), 2100);
    return () => clearInterval(t);
  }, []);

  const active = Math.min(step, steps.length - 1);
  const liveStatus = steps[active]?.status ?? "todo";
  const tickets = baseTickets.map((t) => (t.id === 104 ? { ...t, status: liveStatus } : t));
      ...
      <div className="relative mx-auto grid max-w-6xl gap-10 px-5 py-16 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] lg:items-center lg:py-20">
        <div>
          ...
          <p className="mt-4 max-w-md text-[15px] leading-relaxed text-muted-foreground">
            Axon connects your projects, tickets, teams, and Git activity into one workflow.
          </p>
          <div className="mt-7 flex flex-wrap gap-3">
            <button className="h-9">Get Started →</button>
            <button variant="outline" className="h-9 border-border bg-surface hover:bg-accent">
              Explore Axon
            </button>
          </div>
        ...
        <div className="panel shadow-panel overflow-hidden">
          <div className="flex items-center gap-2 border-b border-border bg-surface-raised px-3 py-2">
            ...
            <span className="size-2 rounded-full bg-border-strong" />
            <span className="size-2 rounded-full bg-border-strong" />
            <span className="ml-2 font-mono text-[11px] text-muted-foreground">Axon Project</span>
                    ...
                    {items.map((t) => (
                      <div
                        ...
                        className={cn(
                          ...
                          t.id === 104 && "border-primary/60 ring-1 ring-primary/25",
                        )}
                      >
                        ...
                        <p className="mt-1 text-[11px] leading-snug text-foreground">{t.title}</p>
          ...
          <div className="border-t border-border bg-card/40 px-3 py-3">
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="h-px flex-1 bg-border" />
              <span className="font-mono text-[10px] text-muted-foreground">axon/api</span>
            </div>
            <ul className="space-y-1.5">
              {steps.map((s, i) => {
                ...
                return (
                  ...
                  >
                    <Icon className={cn("size-3.5", reached ? s.tone : "text-muted-foreground")} />
                    <span className="text-[11px] text-foreground">{s.event}</span>
